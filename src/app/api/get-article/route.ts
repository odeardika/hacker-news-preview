...
export async function GET(req: Request, res: Response) {
  try {
    ...
    const selectedStoryIds = await listStoryIds.then((list) =>
      list.slice(0, 100)
    );
    ...
    const filteredStories = selectedStoryIds.map(async (storyId) => {
      ...
      if (story.url) {
        if (story.type === "story") {
          if (
            story.url.toLowerCase().match(/(github)/) ||
            story.title
              .toLowerCase()
              .match(
                /(software|programming|linux|sql|npm|script|python|pip|open source|open-source|query|db|html)/
              )
          ) {
            return await story;
          }
